...
complementPageviews <- function (df, source.folder, end.date, flag, project.code="en.wikipedia",platform.code="all", wiki="en")
{
  ...
  for (i in 1: nrow(df) )
  {
    ...
    attempts <- 10
    result <- tryCatch({(x * 3)}, error = function(e) e) 
    while (attempts >0 & inherits(result, "error"))
    {
      result <- tryCatch( {
      ...
      {
        print(sprintf("Error for %s ", target))
        attempts <- attempts -1
      }
    }
...
getAveragePageviewsfromAPI <- function (list.titles, start.date, end.date, project.code="en.wikipedia",platform.code="all", wiki="en")
{
  ...
  for (title in list.titles )
  {
    ...
    table.result<- rbind(table.result, data.frame(title=target, avg.pageviews=mean(pageviews$views) ,sd = sd(pageviews$views), max=max(pageviews$views), min=min(pageviews$views)))
  }
